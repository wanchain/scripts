...
mongoimage=$(sudo docker ps -a --filter "name=${mongocontainer}" --format "{{.Image}}"); mongoimage=${mongoimage%$'\n'}; mongoimage=${mongoimage:-"mongo:5.0"}
...
BACKUP_DIR="${workPath}/keystore_back"
...
sudo rm -rf $filebeatConfig/filebeat.yml
echo 'rm $filebeatConfig/filebeat.yml'
touch $filebeatConfig/filebeat.yml
...
output.logstash:
  hosts: ["log.wanchain.org:$( [ "$isTestnet" == true ] && echo "1518" || echo "1519" )"]
